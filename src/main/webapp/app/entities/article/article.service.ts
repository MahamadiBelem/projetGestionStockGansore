...
export class ArticleService {
  ...
  protected convertDateFromClient(article: IArticle): IArticle {
    const copy: IArticle = Object.assign({}, article, {
      dateEnregistrementArticle:
        article.dateEnregistrementArticle && article.dateEnregistrementArticle.isValid()
          ? article.dateEnregistrementArticle.format(DATE_FORMAT)
          : undefined,
    });
    return copy;
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.dateEnregistrementArticle = res.body.dateEnregistrementArticle ? moment(res.body.dateEnregistrementArticle) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((article: IArticle) => {
        article.dateEnregistrementArticle = article.dateEnregistrementArticle ? moment(article.dateEnregistrementArticle) : undefined;
      });
    }
    return res;
  }
